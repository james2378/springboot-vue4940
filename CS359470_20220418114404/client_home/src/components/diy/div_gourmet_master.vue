<template>
	<div class="diy_details_box diy_div_gourmet_master">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/gourmet_master/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/gourmet_master/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "gourmet_master_id",
					source_id: this.obj["gourmet_master_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/gourmet_master/set?gourmet_master_id=' + _this.obj["gourmet_master_id"], {
								praise_len
							}, (res) => {
								if(res.result){
          ...
          if (res.result || res.result === 0) {
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.name_of_talent
					query.img = this.obj.avatar_of_talent
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        vargourmet_master_id = this.obj.gourmet_master_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "gourmet_master",
          source_field: "gourmet_master_id",
          source_id:gourmet_master_id
        }, (res) => {
