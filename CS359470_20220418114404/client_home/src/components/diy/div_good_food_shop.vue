<template>
	<div class="diy_details_box diy_div_good_food_shop">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/good_food_shop/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/good_food_shop/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        vargood_food_shop_id = this.obj.good_food_shop_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "good_food_shop",
          source_field: "good_food_shop_id",
          source_id:good_food_shop_id
        }, (res) => {
