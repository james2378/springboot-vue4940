					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/food_information/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/food_information/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
          ...
          if (res.result || res.result === 0) {
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.img = this.obj.gourmet_cover
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        varfood_information_id = this.obj.food_information_id;
