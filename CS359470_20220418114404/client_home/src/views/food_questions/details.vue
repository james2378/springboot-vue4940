<template>
	<div class="diy_details page_food_questions" id="food_questions_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_food_questions'>
							<div_food_questions :obj="obj"></div_food_questions>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_food_questions from "@/components/diy/div_food_questions.vue";
	...
	export default {
		...
		data(){
			return{
				...
				list_comment: [],
				form_comment: {
					source_table: "food_questions",
					source_field: "food_questions_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/food_questions/set?food_questions_id=' + obj["food_questions_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "food_questions",
							source_field: "food_questions_id",
							source_id: this.obj.food_questions_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.food_questions_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "food_questions",
					source_field: "food_questions_id",
					source_id: obj["food_questions_id"],
					user_id
				};
