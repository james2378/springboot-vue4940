<template>
	<div class="diy_details page_good_food_shop" id="good_food_shop_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_good_food_shop'>
							<div_good_food_shop :obj="obj"></div_good_food_shop>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_good_food_shop from "@/components/diy/div_good_food_shop.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "good_food_shop_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/good_food_shop/set?good_food_shop_id=' + obj["good_food_shop_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "good_food_shop",
							source_field: "good_food_shop_id",
							source_id: this.obj.good_food_shop_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.good_food_shop_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "good_food_shop",
					source_field: "good_food_shop_id",
					source_id: obj["good_food_shop_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.img = this.obj.shop_facade
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/good_food_shop/set?good_food_shop_id=' + _this.obj["good_food_shop_id"], {
								praise_len
							}, (res) => {
								if(res.result){
